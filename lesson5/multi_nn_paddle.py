...
def main():
    ...
    def multi_nn(img,j):
        h1 = paddle.layer.fc(input=img, size =j , act = paddle.activation.Relu())
        h2 = paddle.layer.fc(input=h1, size =j , act = paddle.activation.Relu())
        h3 = paddle.layer.fc(input=h2, size=j, act=paddle.activation.Relu())
        h4 = paddle.layer.fc(input=h3, size=j, act=paddle.activation.Relu())
        y_out = paddle.layer.fc(input = h4, size = 1, act = paddle.activation.Sigmoid())
        return y_out
    j = 7
