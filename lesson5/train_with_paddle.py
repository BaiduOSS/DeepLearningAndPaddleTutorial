...
def network_config():
    ...
    hidden_layer_1 = paddle.layer.fc(
    ...
    hidden_layer_2 = paddle.layer.fc(
        input=hidden_layer_1, size=7, act=paddle.activation.Relu())

    # 隐藏层3，paddle.layer.fc表示全连接层，input=h2: 该层输入数据为h2
    # size=5：神经元个数，act=paddle.activation.Relu()：激活函数为Relu()
    hidden_layer_3 = paddle.layer.fc(
        input=hidden_layer_2, size=5, act=paddle.activation.Relu())
